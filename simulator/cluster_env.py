...
from simulator.job import RLSched_Job
from simulator.job_queue import JobQueue
from utils.average_meter import AverageMeter
from utils.job_launcher import slurm_launch, slurm_preempt, slurm_kill
import simulator.rl_training.job_helper as job_helper
import simulator.rl_training.obs_helper as obs_helper
import simulator.rl_training.reward_helper as reward_helper
import simulator.rl_training.action_helper as action_helper
...
class ClusterEnv(Env):
    def __init__(self, nr_node=4, gpus_per_node=8, rl_algo='PPO', coeff_cont=0.8, coeff_util=0.2, total_jobsets=100, episodes_per_jobset=20, round_dur=300, ckpt_dir=f'{os.environ["DLCM_PATH"]}/simulator/training-ckpt/', trace=f'{os.environ["DLCM_PATH"]}/simulator/traces/generated_trace', isolated_thp_path=f'{os.environ["DLCM_PATH"]}/simulator/jobs/6-workload-isolated-thp.csv', shared_thp_path=f'{os.environ["DLCM_PATH"]}/simulator/jobs/shared_thp_ratio.csv'):
        ...
        # Inference related. Set at start()
        self.scheduler = "rl" # <"rl"/"las"/"srtf">
        self.model = None # if scheduler = "rl", PPO loaded model
                        # else, None
        self.inference = False
        self.simulated_inference = False # If True, do inference without Slurm launch
        self.job_id_to_slurm_job_id_mapper = None # Initialized at start(), if inference && !simulated inference
    ...
    def queue_jobs_from_trace(self, trace):
        ...
        for json_job in job_list:
            job = RLSched_Job(json_job['id'], json_job['model'], json_job['gpu_demand'], self.get_isolated_thp(json_job['model']), json_job['total_samples_to_train'])
            self.job_queue.push(job)

    def get_isolated_thp(self, row, col='isolated thp'):
    ...
    ###
    # (1) Scheduler entry point - either for inference or training.
    def start(self, inference=False, simulated_inference=False, model=None, scheduler="rl"):
        ...
        if inference:
            ...
            # Register ckpt model for inference
            self.model = model

            # Init slurm job id mapper on real cluster inference
            if not simulated_inference:
            ...
            while self.num_jobsets < self.total_jobsets:
                ...
                while self.num_episodes < self.episodes_per_jobset:
                    ...
                    # $DLCM_PATH/simulator/traces/train/0/episode{self.num_episodes}
                    episode_trace = f'{self.trace}/{self.num_jobsets}/episode{self.num_episodes}'
                    ...
                    self.policy.save(f'{self.ckpt_dir}/rl-dlcm-cont{self.coeff_cont}-util{self.coeff_util}-jobset{self.num_jobsets}-episode{self.num_episodes}-ckpt')
        ...
        if self.inference:
            ...
            while valid:
                ...
                if valid:
                    ...
                    _logger.debug(f"[Inference] launch_target_job {launch_target_job}")
                    self.job_queue.remove(job_to_schedule)
                    ...
                    # Launch job to slurm
                    if not self.simulated_inference:
            ...
            if self.inference and not self.simulated_inference:
    ...
    def update_progress(self, job, samples):
        ...
        if job.check_truncate_rule() or job.check_completion():
            ...
            if self.inference and not self.simulated_inference:
                ...
                _logger.debug(f"[Inference] {job} with slurm_job_id {slurm_job_id} exited.")
