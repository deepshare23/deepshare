...
# DLCM: Init system path to `models/DeepSpeedPipelineParallel`
sys.path.append(os.path.join(os.environ['DLCM_PATH'], 'models/DeepSpeedPipelineParallel'))
...
class TestPPTraining:
    ...
    def test_pp_for_sequence_classification(self, num_labels, model_name, pp_stage):
        ...
        dataloader = DataLoader(
            ...
            num_workers=int(os.environ['SLURM_CPUS_PER_TASK']), # DLCM: May cause error if `--cpus-per-task` for sbatch script is not set
            drop_last=True,
