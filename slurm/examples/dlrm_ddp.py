...
""" Modified for DLCM
- L77-86 Imported a few required packages
- L82 Init system path to `models/DLRM`.
- L140-142 Initialized global `_logger` and `args`.
- L1060 Added argument `slurm_handler` to `run()`
- L1061-1064 Setup log level
- L1116-1130 Each process is given rank = `SLURM_PROCID` since Slurm assigns global rank with this environment variable.
            Master address & port is dynamically given via `slurm_handler`. World size is set when Slurm launches the job
- L1575-1595 DLCM profiler (PyTorch profiler, CPU profiler) added
- L1625-1632 DLCM profiler stop condition
- L1954-1958 Parse args, install handler/profiler, launch main code with arguments
- Removed `writer` and `prof` from original code
"""

# DLCM: Additional required packages
import logging
import os
import sys

# DLCM: Init system path to `models/DLRM`
sys.path.append(os.path.join(os.environ['DLCM_PATH'], 'models/DLRM'))

from utils.dlcm_handler import DLCMSlurmHandler
from utils.checkpoint import DummyDLCMJobCheckpointer
from utils.profiler import DLCMProfiler
...
# DLCM: Added argument `slurm_handler` to `run()`
def run(slurm_handler: DLCMSlurmHandler):
    ...
    if not args.debug_mode:
        ...
        # DLCM: Each process is given rank = `SLURM_PROCID`
        # since Slurm assigns global rank with this environment variable.
        # Master address & port is dynamically given via `slurm_handler`.
        # World size is set when Slurm launches the job
        os.environ['MASTER_ADDR'] = f'{slurm_handler.master_addr}'
        os.environ['MASTER_PORT'] = f'{slurm_handler.master_port}'
        os.environ['RANK'] = os.environ['SLURM_PROCID']

        ext_dist.init_distributed(local_rank=args.local_rank, use_gpu=use_gpu, backend="nccl")
        ...
        while k < args.nepochs:
            ...
            for j, inputBatch in enumerate(train_ld):
                ...
                if slurm_handler.profiler != None:
                    slurm_handler.profiler.step(samples=args.mini_batch_size * args.world_size, bt=batch_time)
            # step exit point
...
# DLCM: install handler/profiler, launch main code with arguments
if __name__ == "__main__":
    args = _parse_args()
    slurm_handler = DLCMSlurmHandler(DummyDLCMJobCheckpointer()) # TODO: add DLRM checkpointer
