...
def run(slurm_handler):
    ...
    slurm_handler.print_with_rank("Reached 3") # DLCM TMP
    ...
    slurm_handler.print_with_rank("Reached 4") # DLCM TMP

    torch.manual_seed(12345)
    model = SAGE(dataset.num_features, 256, dataset.num_classes).to(args.local_rank)
    slurm_handler.print_with_rank(f"dataset.num_features[{dataset.num_features}] dataset.num_classes {dataset.num_classes}") # DLCM TMP

    slurm_handler.print_with_rank("Reached 4-1") # DLCM TMP
    slurm_handler.print_with_rank(f"local rank: {args.local_rank}")

    model = DistributedDataParallel(model, device_ids=[args.local_rank])
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

    slurm_handler.print_with_rank("Reached 4-2") # DLCM TMP
    ...
    slurm_handler.print_with_rank("Reached 5") # DLCM TMP
