...
import sys, os
import io
sys.path.append(os.path.join(os.environ['DLCM_PATH'], 'models/fairscale/benchmarks'))
...
def get_lm_model(args, device, config):
    ...
    nhid = config["nhid"]
    # DLCM: Modify read `model_config` value for keys `ndecoder`
...
def benchmark_moe(config_class, args, slurm_handler):
    ...
    # DLCM: Modify read `benchmark_config` value for keys `epochs` and `batch_size`
