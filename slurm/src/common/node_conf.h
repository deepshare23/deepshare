...
typedef struct node_resource_stat {
	float gpu_util;			/* Averaged GPU utilization (%) */
	float cpu_util;			/* Averaged CPU utilization (%) */
	float mem_util;			/* Averaged memory utilization (%) */
	float network_util;		/* Averaged network utilization (%) */
} node_resource_stat_t;
...
struct node_record {
	...
	uint32_t weight;		/* orignal weight, used only for state
					 ...
					 * scheduling purposes. */
	node_resource_stat_t resource_stat;		/* DLCM node monitoring purposes */
};
