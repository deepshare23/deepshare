...
/*
 * Read a float `stat` value from `command` output
 * IN command - shell command to print stat
 * IN stat - pointer to the requested stat
 * RET 0 or error code
 */
static int _read_float_stat_from_command(char* command, float* stat)
{
	FILE* fpipe;
...
{
	char* command = "gpustat | tail -n+2 | awk -F ' ' '{sum+=$7;n++} END {print sum/n;}'";
	return _read_float_stat_from_command(command, gpu_util);
}

extern int get_cpu_util(float *cpu_util)
{	
	char* command = "top -bn2 | grep '%Cpu' | tail -1 | grep -P '(....|...) id,'|awk '{print 100-$8}'";

	return _read_float_stat_from_command(command, cpu_util);
}

extern int get_mem_util(float *mem_util) 
{
	char* command = "free | grep Mem | awk '{print $3/$2 * 100.0}'";

	return _read_float_stat_from_command(command, mem_util);
};

extern int get_network_util(float *network_util)
{
	// TODO: Log system network util
	*network_util = 0.0;
